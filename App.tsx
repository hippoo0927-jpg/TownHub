          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:p-14 lg:pt-4">
            {activeView === 'HOME' ? (
              ...
              <div className="h-full max-w-7xl mx-auto flex flex-col relative">
                ...
                {step === 'EDITOR' && pixelData && (
                  <div className="flex flex-col lg:flex-row gap-0 h-full min-h-0 animate-in fade-in overflow-hidden relative">
                    ...
                    {/* 모바일에서 열렸을 때 배경 어둡게 */}
                    {showPalette && <div className="fixed inset-0 bg-black/60 z-[190] lg:hidden animate-in fade-in" onClick={() => setShowPalette(false)} />}
                  </div>
                )}
              </div>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center gap-10">
                ...
                <div className="font-black italic text-2xl lg:text-5xl uppercase tracking-[0.6em] text-slate-800 animate-pulse">{activeView} HUB LOADING...</div>
              </div>
            )}
          </div>
        </div>
